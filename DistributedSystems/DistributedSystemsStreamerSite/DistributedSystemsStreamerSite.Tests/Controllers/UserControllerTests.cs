    ...
    public class UserControllerTests
    {
        ...
        public void GetAllTest()
        {
            ...
            for(int i = 0; i < test.Count && i < test2.Count; ++i)
            {
                ...
                Assert.AreEqual(test[i].Username, test2[i].Username);
        ...
        public void PostTest()
        {
            ...
            users.Add(new User() { Id = 3, FollowerCount = 40, PageViewCount = 200, SubscriberCount = 0, Username = "Toby" });
            userController.Post(new User() { Id = 3, FollowerCount = 40, PageViewCount = 200, SubscriberCount = 0, Username = "Toby" });

            User user = userController.Get(3);

            Assert.IsNotNull(user);
            Assert.AreEqual(user.Id, users.Last().Id);
            Assert.AreEqual(user.Username, users.Last().Username);
        ...
        public void PutTest()
        {
            ...
            oldTestCase = new User() { Id = oldTestCase.Id, FollowerCount = oldTestCase.FollowerCount, PageViewCount = oldTestCase.PageViewCount, SubscriberCount = oldTestCase.SubscriberCount, Username = oldTestCase.Username, Videos = oldTestCase.Videos};
            User newTestCase = new User() { Id = 3, FollowerCount = 100, PageViewCount = 5000, SubscriberCount = 6, Username = "Tobias" };
            ...
            Assert.AreNotEqual(oldTestCase.Username, userController.Get().Last().Username);
        }
        ...
        public void DeleteTest()
        {
            ...
            Assert.AreNotEqual(deletedUser.Username, userController.Get().Last().Username);
        ...
        public static List<User> TestData()
        {
            ...
            {
                new User() { Id = 0, FollowerCount = 10, PageViewCount = 100, SubscriberCount = 0, Username = "Bob", Videos = new List<Video>() { new Video() { Id = 0, Path = "basicStringPath.mp4", UserId = 0 } } },
                new User() { Id = 1, FollowerCount = 20, PageViewCount = 150, SubscriberCount = 5, Username = "Joe", Videos = new List<Video>() { new Video() { Id = 1, Path = "newStringPath.mp4", UserId = 1 }, new Video() { Id = 2, Path = "anotherStringPath.mp4", UserId = 1 } } },
                new User() { Id = 2, FollowerCount = 50, PageViewCount = 1000, SubscriberCount = 0, Username = "Amber", Videos = new List<Video>() { new Video() { Id = 3, Path = "thirdStringPath.mp4", UserId = 2 } } }
            };
